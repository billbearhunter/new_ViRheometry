...
Usage:
    ...
    python3 run_pipeline.py \\
        ...
        --moe_dir Optimization/moe_workspace5 \\
...
def read_settings(data_dir: Path) -> tuple[float, float]:
...
def main():
    ...
    parser.add_argument("--simulate", action="store_true",
                        help="Run MPM simulation after optimization (Step 4)")
    parser.add_argument("--eta", type=float, default=None,
                        help="Override η for Step 4 (skip optimization result)")
    parser.add_argument("--n", type=float, default=None,
                        help="Override n for Step 4")
    parser.add_argument("--sigma_y", type=float, default=None,
                        help="Override σ_y for Step 4")
    ...
    parser.add_argument("--skip-optimization", action="store_true",
                        help="Skip Step 3 (requires --eta/--n/--sigma_y for Step 4)")
    ...
    # ── Step 3: HB Parameter Optimization ────────────────────────
    ...
    if not args.skip_optimization:
        ...
        result_dirs = sorted((ROOT / "Optimization").glob(
            f"result_setup1_{args.strategy}_*"), reverse=True)
        if result_dirs:
    ...
    if args.simulate:
        ...
        if None in (eta, n, sigma_y):
            print("\n[warn] Step 4 skipped — could not read η/n/σ_y from optimization result.")
            print("       Run manually:")
