// ported from https://github.com/neurolabusc/LewinerMarchingCubes.git
...
public :
  ...
  inline void set_method( const bool originalMC )
  {
    _originalMC = originalMC;
  }

  // Selects to use data from another class.
  //   data is the pointer to the external data, allocated as a size_x*size_y*size_z vector running in x first
  inline void set_ext_data( real* data )
  {
    if( !_ext_data )
    {
      //assert( _data != NULL ); Why does this fail? Strange.
      delete[] _data;  
    }
    _ext_data = data != NULL;
    if( _ext_data )
  ...
  inline real get_data  ( const int i, const int j, const int k ) const { return _data[ i*_size_y*_size_z+j*_size_z+k ] ; }
