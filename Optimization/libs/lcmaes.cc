...
/**
 * The authors of "Non-Newtonian ViRheometry via Similarity Analysis" have modified this code
 * 1. to let the python bind landle "pwqBoundStrategy" and "linScalingStrategy" in order to incorporate the normalization and bounding of the material space
 * 2. to enable the termination of the optimization based on the maximum iteration count in the python bind
 */   
...
void set_x0(const boost::python::list &lbounds,
            const boost::python::list &ubounds,
            CMAParameters<GenoPheno<pwqBoundStrategy,linScalingStrategy>> &parameters)
{
    ...
    parameters.set_x0(&vlbounds.front(), &vubounds.front());
}

template <class TGenoPheno=GenoPheno<pwqBoundStrategy,linScalingStrategy>>
  CMAParameters<TGenoPheno> make_parameters_pwqb_ls(const boost::python::list &x0,
                        const double &sigma,
                        const TGenoPheno &gp,
                        const int &lambda=-1,
...
boost::python::list get_best_candidate_pheno_ls(const CMASolutions &s,
                         const GenoPheno<pwqBoundStrategy,linScalingStrategy> &gp)
{
  boost::python::list gpx;
  dVec d = gp.pheno(s.best_candidate().get_x_dvec());
  for (int i=0;i<d.rows();++i)
...
GenoPheno<pwqBoundStrategy,linScalingStrategy> make_genopheno_pwqb_ls(const boost::python::list &lbounds,
                                                              const boost::python::list &ubounds,
                                                              const int &dim)
{
  assert(len(lbounds)==len(ubounds));
  ...
  return GenoPheno<pwqBoundStrategy,linScalingStrategy>(&vlbounds.front(),&vubounds.front(),dim);
}
...
BOOST_PYTHON_MODULE(lcmaes)
{
  ...
  class_<CMAParameters<GenoPheno<NoBoundStrategy>>>("CMAParametersNB","CMAParameters object for problems with unbounded function parameters")
    ...
    .def("set_stopping_criteria",&CMAParameters<GenoPheno<NoBoundStrategy>>::set_stopping_criteria,"set stopping criteria")
    ;
  ...
  class_<CMAParameters<GenoPheno<pwqBoundStrategy>>>("CMAParametersPB","CMAParameters object for problems with bounded parameters")
    ...
    .def("set_stopping_criteria",&CMAParameters<GenoPheno<pwqBoundStrategy>>::set_stopping_criteria,"set stopping criteria")
    ;
  ...
  class_<CMAParameters<GenoPheno<NoBoundStrategy,linScalingStrategy>>>("CMAParametersNBS","CMAParameters object for problems with scaled parameters")
    ...
    .def("set_stopping_criteria",&CMAParameters<GenoPheno<NoBoundStrategy,linScalingStrategy>>::set_stopping_criteria,"set stopping criteria")
    ;
  ...
  class_<CMAParameters<GenoPheno<pwqBoundStrategy,linScalingStrategy>>>("CMAParametersPBS","CMA Parameters for problems with bounded and rescaled parameters")
    ...
    .def("set_stopping_criteria",&CMAParameters<GenoPheno<pwqBoundStrategy,linScalingStrategy>>::set_stopping_criteria,"set stopping criteria")
    ;
  def("set_x0",set_x0,args("lbounds","ubounds","parameters"),"set_x0");
  def("make_parameters_pwqb_ls",make_parameters_pwqb_ls<GenoPheno<pwqBoundStrategy,linScalingStrategy>>,args("x0","sigma","gp","lambda","seed"),"creates a CMAParametersPBS object for problem with bounded and scaled parameters");
  ...
  def("get_best_candidate_pheno_ls",get_best_candidate_pheno_ls,args("cmasol","gp"),"returns candidate's parameter vector in phenotype space");
  /*- genopheno object -*/
  ...
  class_<GenoPheno<pwqBoundStrategy,linScalingStrategy>>("GenoPhenoPWQBLS","genotype/phenotype transformation object for problem with bounded and scaled parameters")
    ;
  def("make_genopheno_pwqb_ls",make_genopheno_pwqb_ls,args("lbounds","ubounds","dim"),"creates a genotype/phenotype transformation object for problem with bounded and scaled parameters");
