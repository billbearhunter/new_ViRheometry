...
# -*- coding: utf-8 -*-
"""
optimize_2setups_similarity.py  —  joint 2-setup inverse with H^PP preconditioning
==============================================================================
Parallel to `optimize_2setups.py` but with Hamamichi-style similarity
preconditioning. The joint Hessian at the warm-start material guess is
the SUM of per-setup PP Hessians — exactly the quantity the orthogonality
rule tries to make well-conditioned.

Same CLI as optimize_2setups.py (setup1_dir for warm-start prior, etc.).
"""
...
def main():
    ...
    p = argparse.ArgumentParser(
        description="2-setup joint inverse with Hamamichi H^PP preconditioning"
    )
    ...
    p.add_argument("--sigma0",   type=float, default=0.20,
                   help="CMA σ0 in preconditioned z-space (tighter than 1setup "
                        "since warm-start prior is reliable)")
    ...
    save_dir = Path(args.out_dir) if args.out_dir \
               else Path(default_out_root) / f"setup2_sim_{ts}"
    ...
    # ── Warm-start anchor (from setup1 result if available) ─────────────────
    ...
    m_anchor = np.asarray(theta_0, dtype=np.float64)
    # Clip anchor into bounds (singleHessian can blow up at boundary)
    m_anchor = np.clip(m_anchor,
                       [bounds["n"][0], bounds["eta"][0], bounds["sigma_y"][0]],
                       [bounds["n"][1], bounds["eta"][1], bounds["sigma_y"][1]])
    print(f"[Anchor] H^PP pivot: n={m_anchor[0]:.4f}  "
          f"η={m_anchor[1]:.4f}  σ_y={m_anchor[2]:.4f}")

    # Regularisation toward anchor — same formula as baseline 2setup for fair comparison
    scale_n       = MAX_N - MIN_N
    ...
    plt.title(f"Convergence ({len(cfgs)}-setup similarity-preconditioned)")
    plt.xlabel("Iteration"); plt.ylabel("Best Loss")
    plt.savefig(save_dir / "best_loss_convergence.png"); plt.close()

    # Flow-curve comparison per cfg (mirrors baseline)
    try:
        import pandas as pd
        fc_rows = []
        for cfg_idx, cfg in enumerate(cfgs):
